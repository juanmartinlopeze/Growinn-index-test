...
export function useEmpresaData() {
  ...
  const refetch = useCallback(async () => {
    try {
      ...
      if (empresas.length > 0) {
        ...
        const totalAreas = latest.areas || 0;
        ...
        const generatedAreas = Array.from({ length: totalAreas }, (_, i) => {
          const name = `Área ${i + 1}`;
  ...
  return {
    ...
    jerarquiasPlaneadas,
    refetch, // <-- clave para actualizar manualmente desde otros componentes
  };
}
