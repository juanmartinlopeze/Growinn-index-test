// EditRoleModal.jsx
import React from 'react'
import { v4 as uuidv4 } from 'uuid' // npm i uuid
import { useAlert } from '../Alerts/useAlert'
import { Alert, Button, InputDelete } from '../index'
import { InputForm } from '../Inputs/Input-form'
import './Table.css'

export default function EditRoleModal({ title, position, employees, subcargos, onPositionChange, onEmployeesChange, onSubcargosChange, onClose, onSave, onDeleteSubcargo, onDeleteRole }) {
	const [isSaving, setIsSaving] = React.useState(false)

	const { alertInfo, showAlert } = useAlert()

	// Botón “Eliminar subcargo” (servidor + local)
	const handleSubcargoDelete = async (index, subcargo) => {
		const confirmed = await showAlert('delete', 'Eliminar el subcargo', `¿Estás seguro de eliminar "${subcargo.nombre}"? Esta acción no se puede deshacer.`)
		if (!confirmed) return

		try {
			// 1. Borra en BD
			if (subcargo.id) {
				await onDeleteSubcargo(subcargo.id)
			}
			// 2. Borra localmente del array
			const updated = [...subcargos]
			updated.splice(index, 1)
			onSubcargosChange(updated)
			// 3. Mensaje informativo
			showAlert('complete', 'Subcargo eliminado', '✅ Subcargo eliminado correctamente')
		} catch (err) {
			console.error('❌ Error eliminando subcargo:', err)
			showAlert('error', 'Error', '❌ Error al eliminar subcargo')
		}
	}

	// “+ Agregar Subcargo” (solo local, con UID único)
	const handleAddLocalSubcargo = () => {
		const provisional = {
			uid: uuidv4(),
			nombre: '',
			personas: '',
			// cargo_id: ... (no hace falta hasta guardar)
		}
		onSubcargosChange([...subcargos, provisional])
	}

	return (
		<div className='modal-container'>
			<div className='overlay'>
				<div className='modal-content'>
					{alertInfo && <Alert {...alertInfo} position='top-center' onConfirm={alertInfo.onConfirm} onCancel={alertInfo.onCancel} />}
					<form
						onSubmit={async (e) => {
							e.preventDefault()
							setIsSaving(true)
							try {
								await onSave()
							} finally {
								setIsSaving(false)
							}
						}}
					>
						<h3>{title}</h3>

						<div className='input-row'>
							<div className='input-group'>
								<InputForm icon={false} id='position' label='Cargo' value={position} placeholder='Nombre del cargo' onChange={(e) => onPositionChange(e.target.value)} required />
							</div>

							<div className='input-group'>
								<InputForm
									icon={false}
									id='employees'
									label='Empleados'
									type='number'
									value={employees}
									placeholder='Cantidad de empleados'
									onChange={(e) => {
										const num = parseInt(e.target.value, 10)
										onEmployeesChange(isNaN(num) ? 0 : num)
									}}
									required
								/>
							</div>
						</div>

						<div className='divider' />

						<div className='subcargos-section'>
							<label>Subcargos</label>
							{subcargos.map((sub, index) => {
								// Usa key = sub.id si existe; si no, usa sub.uid
								const keyValue = sub.id ?? sub.uid
								return (
									<div key={keyValue} className='subcargo-row'>
										<div className='subcargo-input-group'>
											<InputForm
												icon={false}
												id='employees'
												type='text'
												value={sub.nombre}
												onChange={(e) => {
													const updated = [...subcargos]
													updated[index] = {
														...updated[index],
														nombre: e.target.value,
													}
													onSubcargosChange(updated)
												}}
												placeholder={`Subcargo ${index + 1}`}
												required
											/>
										</div>
										<div className='subcargo-input-group'>
											<InputDelete
												type='number'
												value={sub.personas ?? ''}
												onChange={(e) => {
													const val = parseInt(e.target.value, 10)
													const updated = [...subcargos]
													updated[index] = {
														...updated[index],
														personas: isNaN(val) ? 0 : val,
													}
													onSubcargosChange(updated)
												}}
												placeholder='Cantidad de empleados'
												required
												onClick={() => handleSubcargoDelete(index, sub)}
											/>
										</div>
									</div>
								)
							})}

							<button type='button' className='add-subcargo-link' onClick={handleAddLocalSubcargo}>
								+ Agregar Subcargo
							</button>
						</div>

						<div className='action-buttons'>
							<Button type='submit' variant='submit' className='small-button' disabled={isSaving}>
								{isSaving ? 'Guardando...' : 'Guardar'}
							</Button>
							<Button type='button' variant='cancel' className='small-button' onClick={onClose}>
								Cancelar
							</Button>
							<Button type='button' variant='delete' className='small-button' onClick={onDeleteRole}>
								Eliminar Cargo
							</Button>
						</div>
					</form>
				</div>
			</div>
		</div>
	)
}
