...
// Uses your backend .env (SUPABASE_URL + SUPABASE_ANON_KEY)
...
async function requireAuth(req, res, next) {
	...
	req.user = data.user // make the user available downstream
	next()
}
...
app.post('/empresas', async (req, res) => {
	try {
		...
		if (!nombre || !empleados || !jerarquia1 || !jerarquia2 || !jerarquia3 || !jerarquia4 || !areas || !Array.isArray(areas) || areas.length === 0) {
		...
		const { data: areasData, error: areasError } = await supabaseAdmin.from('areas').insert(areaInserts)

		if (areasError) throw areasError

		// Actualizar campo 'areas' en empresas (opcional)
		const totalAreas = areas.length
		const { error: updateError } = await supabaseAdmin.from('empresas').update({ areas: totalAreas }).eq('id', empresa_id)
		...
		const { data: updatedEmpresa, error: fetchUpdatedError } = await supabaseAdmin.from('empresas').select('*').eq('id', empresa_id).single()
...
app.get('/areas/empresa/:empresaId', async (req, res) => {
	...
	try {
		const { data, error } = await supabase.from('areas').select('*').eq('empresa_id', empresaId)
...
app.post('/areas', async (req, res) => {
	...
	try {
		const { data, error } = await supabaseAdmin.from('areas').insert([{ nombre, empresa_id }]).select().single()
...
app.put('/areas/:id', async (req, res) => {
	...
	try {
		const { data, error } = await supabaseAdmin.from('areas').update({ nombre }).eq('id', id).select().single()
...
app.put('/cargos/:id', async (req, res) => {
	...
	try {
		const { data, error } = await supabaseAdmin.from('cargos').update({ nombre, personas }).eq('id', id).select().single()
...
app.get('/subcargos/cargo/:cargoId', async (req, res) => {
	...
	try {
		const { data, error } = await supabase.from('subcargos').select('*').eq('cargo_id', cargoId)
...
app.delete('/subcargos/:id', async (req, res) => {
	...
	try {
		const { error } = await supabaseAdmin.from('subcargos').delete().eq('id', id)
